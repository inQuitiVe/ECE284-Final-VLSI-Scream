...
For OS (Output Stationary) 2-bit mode:
  - activation_tile*.txt -> viz_activation_tile*.txt (2-bit unsigned values, 8 per row)
  - weight_itile*_otile*_kij*.txt -> viz_weight_*.txt (4-bit signed values, 8 per row)
  - expected_output_from_psum_binary.txt -> viz_expected_output_from_psum_binary.txt (16-bit signed values, 16 per row for 2-bit mode with col=16)
"""
...
def convert_activation_file(input_file, output_file):
    """Convert activation file from binary to decimal format (2-bit unsigned)"""
    try:
        ...
        for line in lines:
            ...
            if not line or line.startswith('#'):
                ...
                # Each line has 16 bits (8 x 2-bit unsigned values)
...
def convert_weight_file(input_file, output_file):
    """Convert weight file from binary to decimal format (4-bit signed)"""
    try:
        with open(input_file, 'r') as f:
            lines = f.readlines()
...
def convert_output_file(input_file, output_file):
    """Convert output file from binary to decimal format (16-bit signed, 16 per row)"""
    try:
        with open(input_file, 'r') as f:
            lines = f.readlines()
...
def main():
    ...
    output_file = os.path.join(source_dir, "expected_output_from_psum_binary.txt")
    if os.path.exists(output_file):
        total_count += 1
        output_path = os.path.join(output_dir, "viz_expected_output_from_psum_binary.txt")
        if convert_output_file(output_file, output_path):
            print(f"  [OK] expected_output_from_psum_binary.txt -> viz_expected_output_from_psum_binary.txt")
            success_count += 1
        else:
            print(f"  [FAIL] expected_output_from_psum_binary.txt")
