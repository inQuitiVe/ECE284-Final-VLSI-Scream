...
For OS (Output Stationary) 4-bit mode:
  - activation_tile*.txt -> viz_activation_tile*.txt (4-bit unsigned values, 8 per row)
  - weight_itile*_otile*_kij*.txt -> viz_weight_*.txt (4-bit signed values, 8 per row)
  - out.txt -> viz_out.txt (16-bit signed values, 8 per row for 4-bit mode with col=8)
  - out_raw.txt -> viz_out_raw.txt (16-bit signed values, 8 per row)
"""
...
def convert_activation_file(input_file, output_file):
    """Convert activation file from binary to decimal format (4-bit unsigned)"""
    try:
        ...
        for line in lines:
            ...
            if not line or line.startswith('#'):
                ...
                # Each line has 32 bits (8 x 4-bit unsigned values)
...
def convert_output_file(input_file, output_file):
    """Convert output file from binary to decimal format (16-bit signed, 8 per row)"""
    try:
        with open(input_file, 'r') as f:
            lines = f.readlines()
        ...
        for line in lines:
            ...
            if not line or line.startswith('#'):
                ...
                # Each line has 128 bits (8 x 16-bit signed values for 4-bit mode with col=8)
                # Convert to 8 signed decimal values
                values = []
                for i in range(8):
                    start_idx = i * 16
...
def main():
    ...
    output_files = ["out.txt", "out_raw.txt"]
    for output_filename in output_files:
        output_file = os.path.join(source_dir, output_filename)
        if os.path.exists(output_file):
            total_count += 1
            output_path = os.path.join(output_dir, f"viz_{output_filename}")
            if convert_output_file(output_file, output_path):
                print(f"  [OK] {output_filename} -> viz_{output_filename}")
                success_count += 1
            else:
